...
object ScalaTeraSort {
  ...
  (rdd: ProvRDD[(K, V)]) = new ConfigurableOrderedRDDFunctions[K, V, (K, V)](rdd)
  ...
  def main(args: Array[String]) {
    ...
    val data = new ProvRDD(sc.newAPIHadoopFile[Text, Text, TeraInputFormat](args(0))).map {
    ...
    val sorted_data = new ProvRDD(ordered_data.sortByKeyWithPartitioner(partitioner = partitioner)).map{case (k, v)=>(new Text(k), new Text(v))}
