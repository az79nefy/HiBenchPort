...
object GraphxNWeight extends Serializable{
  ...
  def nweight(sc: SparkContext, input: String, output: String, step: Int,
    ...
    val edges = new ProvRDD(sc.textFile(input, numPartitions)).flatMap { line =>
    ...
    var msg: ProvRDD[(VertexId, Long2DoubleOpenHashMap)] = null
    for (i <- 2 to step) {
      msg = new ProvRDD(g.aggregateMessages(mapF, reduceF))
      g = g.outerJoinVertices(msg)(updateF).persist(storageLevel)
    }

    new ProvRDD(g.vertices).map { case (vid, vdata) =>
