...
// ESpark imports
import org.apache.spark.rdd.ProvRDD.rddToNewPairRDDFunctions
import org.apache.spark.rdd.ProvRDD.rddToRDDProvFunctions
import org.apache.spark.rdd.RDD.rddToPairRDDFunctions

// Often direct ProvRDD import also needed
...
object ALSExample {
  ...
  def run(params: Params): Unit = {
    ...
    val rawdata: ProvRDD[Vector] = new ProvRDD(sc.objectFile(params.dataPath))
    val data: ProvRDD[Rating] = new ProvRDD(Vector2Rating(rawdata, numUsers, numProducts))
    val training: ProvRDD[Rating] = new ProvRDD(data.randomSplit(Array(1-params.sparsity, params.sparsity))(0)).cache()
    val testsparsity = 0.75 * (1-params.sparsity)
    val test: ProvRDD[Rating] = new ProvRDD(data.randomSplit(Array(1-testsparsity, testsparsity))(0))
  ...
  def computeRmse(model: MatrixFactorizationModel, data: ProvRDD[Rating], implicitPrefs: Boolean)
    : Double = {
    ...
    val predictions: ProvRDD[Rating] = new ProvRDD(model.predict(data.map(x => (x.user, x.product))))
  ...
  def Vector2Rating(rawdata: ProvRDD[Vector], m: Int, n: Int)
    : ProvRDD[Rating] = {
    
    val Ratingdata: ProvRDD[Rating] = new ProvRDD(rawdata.zipWithIndex()).flatMap(v => {
