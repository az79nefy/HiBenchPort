...
//    Logger.getRootLogger.setLevel(Level.WARN)
    ...
    val data = new ProvRDD(sc.sequenceFile[Text, Text](params.input)).map{case (k, v) => (k.toString, v.toString)}
    val wordCount = data
      .flatMap{ case (key, doc) => doc.split(" ")}
      .map((_, 1L))
      .reduceByKey(_ + _)
    val wordSum = wordCount.map(_._2).reduce(_ + _)
    val wordDict = new ProvRDD(wordCount.zipWithIndex())
    ...
    val training = new ProvRDD(splits(0))
    val test = new ProvRDD(splits(1))
    ...
    val prediction = new ProvRDD(model.predict(test.map(_.features)))
    val predictionAndLabel = new ProvRDD(prediction.zip(test.map(_.label)))
